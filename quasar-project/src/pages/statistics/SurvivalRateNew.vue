<template>
  <q-page padding class="q-gutter-y-md">
    <h5 class="text-bold text-center"> 신규 유저 잔존율 조회 </h5>
    <!-- 날짜 선택-->
    ...
    <q-table
      title="신규 유저 잔존율 목록"
      :rows="statisticsStore.newUserRetention"
      :columns="columns"
    ...
    >
      <template #body="props">
        <q-tr :props="props">
          <q-td key="regdate" :props="props">{{ props.row.regdate }}</q-td>
          <q-td key="total_new_users" :props="props">{{ props.row.total_new_users }}</q-td>
...
const columns = [
  { name: 'regdate', label: '생성 일자', field: 'regdate', align: 'center', sortable: true },
  { name: 'total_new_users', label: '신규 유저수(NRU)', field: 'total_new_users', align: 'center', sortable: true },
...
const exportToExcel = () => {
  const ws = utils.json_to_sheet(statisticsStore.newUserRetention);
  const wb = utils.book_new();
  utils.book_append_sheet(wb, ws, "신규 유저 잔존율");
  const filename = `신규 유저 잔존율 ${startDate.value}.xlsx`;
  writeFile(wb, filename);
}

const { newUserRetention } = statisticsStore;

const chartLabels = (data) => {
  return data.map(item => item.regdate);
};
...
const chartData = ref({ 
  labels: chartLabels(newUserRetention),
  datasets: chartDatasets(newUserRetention)
});

watch(() => statisticsStore.newUserRetention, (newVal, oldVal) => {
  if (newVal !== oldVal) {
    chartData.value = {
      labels: chartLabels(newUserRetention),
      datasets: chartDatasets(newUserRetention)
...
const fetchData = async () => {
  const data = await statisticsStore.loadNewUserRetention({ start: startDate.value, end: endDate.value });
...
const clearData = () => {
  statisticsStore.newUserRetention = [];
};
</script>
